...
class ImageMultiTextDataset(DistLineReadingDataset):
    def __init__(self, config, data_path, rank=0, world_size=1, shuffle=True, repeat=True, transform=None, config_key='images'):
        ...
        if 'language_chosen' in config[config_key].keys():
            assert isinstance(config[config_key]['language_chosen'], list)
            self.language_chosen = set(config[config_key]['language_chosen'])
    ...
    def get_caption(self, captions, language='', return_keys=False):
        ...
        if isinstance(captions, str):  # compatible to my monolingual data
            if return_keys:
                return captions, 'en'
            return captions
...
class RegionMultiTextDataset(ImageMultiTextDataset):
    def __init__(self, config, data_path, rank=0, world_size=1, shuffle=True, repeat=True, transform=None,
                 box_transform=None, config_key='regions'):
        ...
        self.code_switch = config[config_key].get('code_switch', False)

        if self.careful_hflip:
...
class ParaTextDataset(DistLineReadingDataset):
    def __init__(self, config, data_path, rank=0, world_size=1, shuffle=True, repeat=True, config_key='mtexts'):
        super().__init__(data_path, rank, world_size, shuffle, repeat)

        self.source_key = config[config_key]['source_key']
        self.target_key = config[config_key]['target_key']
        self.tokenized = config[config_key]['tokenized']
        ...
        if 'max_words' in config[config_key]:
            self.max_words = config[config_key]['max_words']
            self.max_tokens = config[config_key]['max_tokens']
            self.mask_prob = config[config_key]['mask_prob']
            self.max_masks = config[config_key]['max_masks']
        else:
            self.max_words = config['max_words']
            self.max_tokens = config['max_tokens']
        ...
        self.use_tlm = config[config_key].get('use_tlm', False)
