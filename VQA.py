...
def calculate_acc(result_rpath, test_dataset):
    ans_key = config.get('ans_key', 'answer')
    index_key = config.get('index_key', 'question_id')

    gt = {}
    for ann in test_dataset.ann:
        if ans_key in ann.keys():
            gt[ann[index_key]] = ann[ans_key].strip()
        else:
            return
    ...
    with open(result_rpath, 'r') as f:
        for sample in json.load(f):
            ...
            if sample[ans_key].strip() == gt[index]:
                n_correct += 1
...
def main(args, config):
    ...
    if args.evaluate:
        ...
        result_rpath = collect_result(vqa_result, 'vqa_eval', local_wdir=args.result_dir,
                                      ...
                                      write_to_hdfs=len(args.output_hdfs)>0, save_result=True)
        ...
        for epoch in range(start_epoch, max_epoch):
            ...
            if epoch >= config['start_eval']:
                vqa_result = evaluation(model, test_loader, tokenizer, device, config)
                result_rpath = collect_result(vqa_result, 'vqa_result_epoch%d' % epoch, local_wdir=args.result_dir, hdfs_wdir=args.output_hdfs,
                                         write_to_hdfs=len(args.output_hdfs)>0, save_result=True)
