    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if (self.local_attn_depth > 0) and (idx >= self.depth - self.local_attn_depth):
                ...
                layer_outputs = encoder_layer(hidden_states, attention_mask=image_atts_blk,output_attentions=output_attentions)
                hidden_states = layer_outputs[0]
                if output_attentions:
                    all_attentions = all_attentions + (layer_outputs[1],)
            else:
                layer_outputs = encoder_layer(hidden_states, attention_mask=None,output_attentions=output_attentions)
                hidden_states = layer_outputs[0]
                if output_attentions:
                    all_attentions = all_attentions + (layer_outputs[1],)
        ...
        outputs = (hidden_states,encoder_states,all_attentions)
...
class CLIPVisionTransformer(nn.Module):
    ...
    def forward(
        ...
        outputs = self.post_layernorm(last_hidden_state)

        if idx_to_group_img is not None:
            bs = len(idx_to_group_img)
            outputs, outputs_fullatts = torch.split(outputs, [bs, outputs.size(0)-bs])
            return (outputs,encoder_outputs[1],encoder_outputs[2],outputs_fullatts)

        return (outputs,encoder_outputs[1],encoder_outputs[2])
